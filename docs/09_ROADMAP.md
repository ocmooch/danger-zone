...
| P12 | Injury reports (Phase 1) | 1-2 hr | `player_injury_reports` table + nflverse crawler + backfill |
...
- [x] `ff-pipeline verify` passes for at least 3 known good weeks per season — `verify --sweep` runs weeks 1/8/15 for every starter (tol 0.1) against NFL.com gamecenter totals. Full 2016–2025 sweep after reconstruction + identity merge: **2281/2880 (79.2%)** exact matches. Every residual failure is one of three *documented data-source/identity gaps*, not an engine or rule error: (a) **322** team-DST starters had no nflverse team-defense rollup — **now resolved**: `crawlers/nflverse/team_defense.py` rolls team stats + schedules into DST stat dicts that the existing engine scores, ingested via `ff-pipeline team-defense` + `rescore` (the 79.2% figure above predates that ingest run; re-sweep after it lands to pick the DST starters up); (b) **~121** historical starters resolve to a statless identity stub — mostly DST and obscure/non-nflverse names, plus ~49 genuinely ambiguous abbreviated names that the conservative identity merge declines to guess (§P1-V3); (c) **156** were the 40+/50+ yard-TD-length bonuses, now resolved by play-by-play derivation into `player_stats_raw` (§P1-V2). A 2025 team-total reconcile after re-ingest/rescore improved from **107/196** passing to **173/196** passing; the remaining **23** residual team-weeks are classified under the reconstruction/team-total trust-check (§P1-V5).
...
## P12 — Injury reports (Phase 1)

**Goal**: surface NFL injury report designations (Out / Doubtful / Questionable / Probable) in the database so the Phase 2 BFF box score can explain why a player scored 0.

**Source**: nflverse `load_injuries(seasons)` via nflreadpy. Keyed by `gsis_id`. Available from 2009 onward. Confirmed columns: `season`, `game_type`, `week`, `gsis_id`, `report_primary_injury`, `report_secondary_injury`, `report_status`, `practice_primary_injury`, `practice_secondary_injury`, `practice_status`, `date_modified`.

**Tasks**:
- `repository/models.py`: `PlayerInjuryReport` model + migration `407521092d7e`
- `crawlers/nflverse/client.py`: `load_injuries` added to `NflverseSource` protocol + `LiveNflverseSource` + `LocalParquetSource`
- `crawlers/nflverse/injury_runner.py`: `run_injury_reports(session, seasons, source)` + `NflverseInjuryRunResult`
- `repository/queries.py`: `injury_reports_for_week(session, season_year, week) → dict[int, PlayerInjuryReport]`
- `cli.py`: `ff-pipeline injury-reports [--season | --start-year / --end-year]`
- `scripts/backfill_injury_reports.py`: repeatable backfill for 2009–2025
- `tests/unit/test_injury_runner.py`: 6 unit tests; no network, in-memory DB

**Done when**:
- [x] `player_injury_reports` table exists with correct schema and unique constraint — migration `407521092d7e` autogenerated and verified
- [x] `injury_reports_for_week()` tested and returns correct `player_id → row` mapping
- [x] Full test gate green (`pytest`, `ruff check`, `ruff format --check`, `mypy`)
- [ ] `scripts/backfill_injury_reports.py` run against `data/fantasy.db`; 2009–2025 populated (expect ~5000–8000 rows/REG season)
- `docs/04_DATA_MODEL.md` updated with `player_injury_reports` table entry

Phase 2 follow-on: dz-dashboard BFF reads `injury_reports_for_week()` and the frontend renders the designation badge next to the player's box score row.
